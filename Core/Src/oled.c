...
{
    uint8_t char_width = size / 2; // ASCII字符宽度
    ...
    while ((*p >= ' ') && (*p <= '~') && (*p != '\0'))
    {
        //仅在需要判断换行时，计算当前单词的宽度，而不是整个剩余字符串
        if (*p == ' ')
        {
            // 如果是空格，直接计算空格宽度
        ...
        {
            // 如果不是空格，向后遍历直到遇到空格或字符串结束，计算当前单词的宽度
            temp = p;
            word_width = 0;
        ...
        // 2. 宽度越界判断：如果当前坐标 + 当前单词宽度 > 屏幕宽度，则换行
        // 注意：如果单词本身就比屏幕还宽，这里也会强制换行，避免死循环
