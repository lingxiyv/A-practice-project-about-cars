...
- **编码器调速**：旋转编码器通过 EXTI 边沿中断解码，实时增减 PWM 占空比，控制电机转速
- **OLED 显示**：实时显示车辆状态、PWM 值与编码器脉冲计数
...
- **蓝牙响应导致 PWM 值异常下降**：每次蓝牙接收指令并响应时，用于存储 PWM 占空比的变量（`main.c` 中的 `pwm_value`）可能下降 50~200 不等。该变量由编码器 EXTI 中断回调（`HAL_GPIO_EXTI_Callback`）维护，蓝牙 UART 中断响应期间可能出现偶发干扰，导致占空比被误减，具体原因待进一步排查。
